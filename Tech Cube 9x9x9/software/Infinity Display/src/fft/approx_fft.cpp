...
unsigned char isin_data[128] = {
    ...
    19,  20,  22,  23,  24,  26,  27,  28,  29,  31,  32,  33,  35,  36,  37,
    39,  40,  41,  42,  44,  45,  46,  48,  49,  50,  52,  53,  54,  56,  57,
    59,  60,  61,  63,  64,  65,  67,  68,  70,  71,  72,  74,  75,  77,  78,
    80,  81,  82,  84,  85,  87,  88,  90,  91,  93,  94,  96,  97,  99,  100,
    102, 104, 105, 107, 108, 110, 112, 113, 115, 117, 118, 120, 122, 124, 125,
    127, 129, 131, 133, 134, 136, 138, 140, 142, 144, 146, 148, 150, 152, 155,
    157, 159, 161, 164, 166, 169, 171, 174, 176, 179, 182, 185, 188, 191, 195,
    198, 202, 206, 210, 215, 221, 227, 236};
unsigned int Pow2[14] = {1,   2,   4,   8,    16,   32,  64,
                         128, 256, 512, 1024, 2048, 4096};
unsigned char RSSdata[20] = {7, 6, 6, 5, 5, 5, 4, 4, 4, 4,
                             3, 3, 3, 3, 3, 3, 3, 2, 2, 2};
//---------------------------------------------------------------------------------//
...
1. in[]     : Data array,
2. N        : Number of sample (recommended sample size
...
i.e, for 150 number of samples, code will consider first 128 sample, remaining
sample  will be omitted. For Arduino nano, FFT of more than 256 sample not
possible due to mamory limitation Code by ABHILASH Contact:
abhilashpatel121@gmail.com Documentation & details:
https://www.instructables.com/member/abhilash_patel/instructables/
*/

float Approx_FFT(int in[], int out_r[], int out_im[], unsigned int N,
                 float Frequency) {
  ...
  for (int i = 0; i < o; i++)  // fft
  {
    ...
    for (int j = 0; j < i10; j++) {
      ...
      for (int k = 0; k < i11; k++) {
        ...
        if (c == 0) {
          ...
          tr = fast_cosine(out_r[temp4], c) -
               fast_sine(out_im[temp4],
                         c);  // the fast sine/cosine function gives direct
                              // (approx) output for A*sinx
    ...
    if (check == 1) {  // scalling the matrics if value higher than 15000 to
                       // prevent varible from overflowing
      for (int i = 0; i < a; i++) {
        out_r[i] = out_r[i] >> 1;
        out_im[i] = out_im[i] >> 1;
      }
  ...
  if (scale > 128) {
    ...
    scale = 128 - scale;
  }  // in case of nnumber getting higher than 32000, we will represent in as
     // multiple of 2^scale

  /*
for(int i=0;i<a;i++)
{
Serial.print(out_r[i]);Serial.print("\t");                    // un comment to
print RAW o/p Serial.print(out_im[i]); Serial.print("i");Serial.print("\t");
Serial.print("*2^");Serial.println(scale);
}
*/

  //---> here onward out_r contains amplitude and our_in conntains frequency
  //(Hz)
  ...
  for (unsigned int i = 1; i < Pow2[o - 1];
       i++)  // getting amplitude from compex number
  {
...
int fast_sine(int Amp, int th) {
  ...
  accuracy =
      5;  // set it value from 1 to 7, where 7 being most accurate but slowest
