...
// Define this to use reciprocal multiplication for division and some more
// speedups that might decrease precision
// #define FFT_SPEED_OVER_PRECISION

// Define this to use a low-precision square root approximation instead of the
// regular sqrt() call This might only work for specific use cases, but is
// significantly faster. Only works for ArduinoFFT<float>.
...
class ArduinoFFT {
 public:
  ...
  ArduinoFFT(T *vReal, T *vImag, uint_fast16_t samples, T samplingFrequency,
             T *windowWeighingFactors = nullptr)
      : _vReal(vReal),
        _vImag(vImag),
        _samples(samples)
  ...
  void complexToMagnitude() const {
    ...
    for (uint_fast16_t i = 0; i < this->_samples; i++) {
      this->_vReal[i] =
          sqrt_internal(sq(this->_vReal[i]) + sq(this->_vImag[i]));
    }
  }
  ...
  void windowing(FFTWindow windowType, FFTDirection dir,
    ...
    if (_windowWeighingFactors && _weighingFactorsComputed &&
      ...
      if (dir == FFTDirection::Forward) {
        ...
        for (uint_fast16_t i = 0; i < (this->_samples >> 1); i++) {
#ifdef FFT_SPEED_OVER_PRECISION
          // on many architectures reciprocals and multiplying are much faster
          // than division
 ...
 private:
  ...
  static inline V sqrt_internal(
      typename std::enable_if<std::is_same<V, float>::value, V>::type x) {
    ...
    u.x = (0.125 * 3.0) * xu *
          (5.0 - xu2 * ((10.0 / 3.0) -
                        xu2));  // Halley's method, repeating increases accuracy
    return u.x;
  }

  template <typename V = T>
  static inline V sqrt_internal(
      typename std::enable_if<std::is_same<V, double>::value, V>::type x) {
    // According to HosrtBaerbel, on the ESP32 the approximation is not faster,
    // so we use the standard function
#ifdef ESP32
    return sqrt(x);
#else
    union  // get bits for float value
