...
#define SAMPLING_FREQ \
  40000  // Hz, must be 40000 or less due to ADC conversion time. Determines
         // maximum frequency that can be analysed by the FFT Fmax=sampleF/2.
#define AMPLITUDE \
  1000  // Depending on your audio source level, you may need to alter this
...
const int BRIGHTNESS_SETTINGS[3] = {
    5, 70, 200};     // 3 Integer array for 3 brightness settings (based on
...
#define NOISE \
  500  // Used as a crude noise filter, values below this are ignored
const uint8_t kMatrixWidth = 16;                 // Matrix width
const uint8_t kMatrixHeight = 16;                // Matrix height
#define NUM_LEDS (kMatrixWidth * kMatrixHeight)  // Total number of LEDs
#define BAR_WIDTH \
  (kMatrixWidth / (NUM_BANDS - 1))  // If width >= 8 light 1 LED width per bar,
...
void loop() {
  // Don't clear screen if waterfall pattern, be sure to change this is you
  // change the patterns / order
  ...
  for (int i = 0; i < SAMPLES; i++) {
    newTime = micros();
    vReal[i] =
        analogRead(AUDIO_IN_PIN);  // A conversion takes about 9.7uS on an ESP32
    vImag[i] = 0;
...
void changingBars(int band, int barHeight) {
  ...
  for (int x = xStart; x < xStart + BAR_WIDTH; x++) {
    for (int y = TOP; y >= TOP - barHeight; y--) {
      matrix->drawPixel(x, y,
                        CHSV(y * (255 / kMatrixHeight) + colorTimer, 255, 255));
    }
  }
}
...
void outrunPeak(int band) {
  ...
  for (int x = xStart; x < xStart + BAR_WIDTH; x++) {
    matrix->drawPixel(
        x, peakHeight,
        ColorFromPalette(outrunPal, peakHeight * (255 / kMatrixHeight)));
  }
}
