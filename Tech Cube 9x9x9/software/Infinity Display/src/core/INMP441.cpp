...
uint16_t loop(int16_t *sample_data, const uint16_t samples) {
  ...
  while (samples_read < samples) {
    size_t bytes_read = 0;
    size_t bytes_to_read = (samples - samples_read) << 2;
    if (bytes_to_read > sizeof(buffer)) {
      bytes_to_read = sizeof(buffer);
    }
    i2s_read(I2S_PORT, buffer, bytes_to_read, &bytes_read, portMAX_DELAY);
    // Convert 24 bits (left aligned inside 32 bits) to 16 bits with some gain
    for (uint16_t i = 0; i < bytes_read >> 2; i++) {
      // TODO: Implement some form of gain control to prevent cutting off data
      sample_data[samples_read++] = buffer[i] >> 12;
    }
  }
  return samples_read;
}
