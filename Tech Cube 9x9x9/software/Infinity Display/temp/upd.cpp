...
void I2SSetup() {
  ...
  const i2s_config_t i2s_config = {
      .mode = i2s_mode_t(I2S_MODE_MASTER |
                         I2S_MODE_RX),  // We drive clk but only receive
                                        // 112000 and below seem to be cleaner
      .sample_rate = 112000,  // 16000, 32000, 44100, 48000, 96000, 112000,
                              // 128000, 144000, 160000, 176000, 192000
      .bits_per_sample =
          I2S_BITS_PER_SAMPLE_32BIT,  // WS signal must be BCLK/64 - this is how
                                      // we manage it
  ...
  REG_SET_BIT(I2S_CONF_REG(I2S_PORT),
              I2S_RX_MSB_SHIFT);  // Phillips I2S - WS changes a cycle earlier
...
void mic_record() {
  int num_bytes_read = i2s_read_bytes(I2S_PORT, (char*)buffer + rPtr,
  ...
  if (rPtr > 2043) rPtr = 0;
    ...
    swap t1, t2;
    t2.longword = buffer[i] >> 8;
    t1.byte[0] = t2.byte[3];
    t1.byte[1] = t2.byte[2];
    t1.byte[2] = t2.byte[1];
    t1.byte[3] = t2.byte[0];
    buffer[i] = t1.longword;
  }
