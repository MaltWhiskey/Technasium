 ...
 *----------------------------------------------------------------------------*/
static inline uint8_t mmap8(uint8_t in, uint8_t inMin, uint8_t inMax,
...
static inline uint8_t mmap8(uint8_t scalar, uint8_t outMin, uint8_t outMax) {
  if (outMax > outMin)
    return outMin + (((outMax - outMin + 1) * scalar) >> 8);
  else
    return outMin - (((outMin - outMax + 1) * scalar) >> 8);
}
static inline uint8_t mmap8(uint8_t scalar, uint8_t outMax) {
  return (((outMax + 1) * scalar) >> 8);
}
static inline float mmapf(float in, float inMin, float inMax, float outMin,
                         float outMax) {
  return (outMax - outMin) * (in - inMin) / (inMax - inMin) + outMin;
}
static inline uint8_t mqadd8(uint8_t a, uint8_t b) {
  uint16_t t = a + b;
  if (t > 255) return 255;
  return t;
}
static inline uint8_t mqsub8(uint8_t a, uint8_t b) {
  if (b > a) return 0;
  return a - b;
}
static inline uint8_t mscale8(uint8_t i, uint8_t scale) {
...
static inline uint8_t mblend8(uint8_t amount_b, uint8_t a, uint8_t b) {
  uint16_t partial;
  partial = (a << 8) | b;
  partial += (b * amount_b);
  partial -= (a * amount_b);
